...
namespace Client
{
    public class ServerSide
    {
        ...
        public List<ServerShipB> ProtectedShips = new List<ServerShipB>();
        ...
        public ServerSide(SideBattleParam param, ShipSide side, ServerBattle battle)
        {
            ...
            {
                byte shiphex = 255; if (param.ShipsHexes.Count > shipid) shiphex = param.ShipsHexes[shipid];
                if (sp.Hex == 255) sp.Hex = shiphex;
                Ships.Add(shipid, ServerShipB.GetStoryShip(shipid, side, battle, sp));
                if (sp.Protected == true) ProtectedShips.Add(Ships[shipid]);
                shipid++;
            }
            if (param.IsRessurect) IsResurrect = true;
            IsAutoControl = true;
        }
        public ServerSide(ServerFleet fleet, SideBattleParam param, ShipSide side, ServerBattle battle)
        {
            CurBattle = battle;
            ...
            byte portalhex = 255; if (param!=null && param.ShipsHexes.Count > 0) portalhex = param.ShipsHexes[0];
            Ships.Add(0, ServerShipB.GetPortalShip(0, warphealth, SSide, battle, portalhex, Emblem));
            //Ships.Add(0, new Portal(SSide, warphealth, battle));
            byte shipid = 0;
            foreach (ServerShip ship in fleet.Ships.Values)
            {
                shipid++;
                byte shiphex = 255; if (param!=null && param.ShipsHexes.Count > shipid) shiphex = param.ShipsHexes[shipid];
                Ships.Add(shipid, new ServerShipB(ship, shipid, SSide, battle, shiphex));
            }
            if (param!=null) foreach (ShipBattleParam sp in param.Ships)
            {
                shipid++;
                Ships.Add(shipid, ServerShipB.GetStoryShip(shipid, side, battle, sp));
                if (sp.Protected == true) ProtectedShips.Add(Ships[shipid]);
            }
            if (param!=null && param.IsRessurect) IsResurrect = true;
        ...
        /// <summary> метод проверяет, потерпела ли сторона поражение из-за потери всех защищаемых кораблей </summary>
        public bool CheckProtectedLose()
        {
            if (ProtectedShips.Count == 0) return false;
            foreach (ServerShipB ship in ProtectedShips)
                if (ship.Hex <= CurBattle.Field.MaxHex)
                    return false;
            return true;
        }
            ...
            for (;;)
            {
                ...
                switch (command.Array[0])
                {
                    case 4:
                        ...
                        else { reason = "Ошибка при выходе на поле боя"; return reason; }
                    case 5:
                        move = TryMoveShip(command.Array[1], command.Array[2]);
                        if (move.IsTrue) { CurBattle.Moves.AddMove(move); return ""; }
                        else
                        { reason = "Ошибка при передвижении"; return reason; }
                    case 6:
                        move = TryRotateShip(command.Array[1], command.Array[2]);
                        if (move.IsTrue) { CurBattle.Moves.AddMove(move); return ""; }
                        else { reason = "Ошибка при повороте"; return reason; }
                    case 7:
                    case 8:
                    case 9:
                        shootfalsereason = "";
                        GameMove[] ShootMoves = TryShootToEnemy(command.Array[1], command.Array[2], command.Array[0] - 7);
                        if (ShootMoves == null) { reason = "Ошибка при выстреле"; return reason; }
                        else
                        {
                    ...
                    case 10:
                        ...
                        if (move == null)
                        {reason = "Ошибка при возврате"; return reason;  }
                        CurBattle.Moves.AddMove(move);
                        return "";
                    case 32:
                        if (UseArtefact(0, command.Array[1], command.Array[2]) == false)
                        { reason = "Ошибка при использовании артефакта"; return reason; }
                        else return "";
                    case 33:
        ...
        public bool UseArtefact(byte pos, byte hex1, byte hex2)
        {
            ...
            switch(Artefacts[pos].Artefact.Ability)
            {
                case ArtefactAbility.BAsteroidMove: if (CurBattle.Asteroids.ContainsKey(hex1)==false) { artefactusefalsereason = "На этом месте нет астероида"; return false; }
            ...
            Artefacts[pos].WaitTime = Artefacts[pos].Artefact.WaitTime;
            ...
            switch (Artefacts[pos].Artefact.Ability)
            {
                ...
                case ArtefactAbility.BPortalMove:
                    ServerShipB movedportal = CurBattle.BattleField[hex1];
                    Hex targethexmoveportal = CurBattle.Field.Hexes[hex2];
                    movedportal.SetHex(hex2);
                    CurBattle.BattleField.Remove(hex1);
                ...
                case ArtefactAbility.BEnergyDamage:
                    ...
                    List<ServerShipB> PassiveTargets = new List<ServerShipB>();
                    foreach (ServerShipB ship in CurBattle.BattleField.Values)
                        if (ship.States.Indestructible == false) PassiveTargets.Add(ship);
                    List<byte> LightJumpHexes = new List<byte>();
                    LightJumpHexes.Add(CurBattle.Field.Hexes[hex1].ID);
                    byte LastPoint = Hex1.ID;
                    int selecttargets = 0;
                    if (CurBattle.BattleField.ContainsKey(hex1) && CurBattle.BattleField[hex1].States.Indestructible == false)
                    { Targets.Add(CurBattle.BattleField[hex1]); PassiveTargets.Remove(Targets[0]); selecttargets++; }
                    for (int i = selecttargets; i < 5; i++)
                    {
                        if (PassiveTargets.Count == 0) break;
                        ServerShipB NextTarget = PassiveTargets[0]; double distance = Double.MaxValue;
                        foreach (ServerShipB ship in PassiveTargets)
                        {
                            byte[] intersecthexes = IntBoya.Battle.Field.IntersectHexes[LastPoint, ship.Hex, 1];
                            bool errortarget = false;
                            foreach (byte b in intersecthexes)
                            {
                                if (CurBattle.BattleField.ContainsKey(b) && CurBattle.BattleField[b].States.Side == ShipSide.Neutral) { errortarget = true; break; }
                            }
                            if (errortarget == true) continue;
                            double d = CurBattle.Field.DistAngleRot[LastPoint, ship.Hex, 1, 0];
                            if (d < distance) { NextTarget = ship; distance = d; }
                        }
                        if (distance == Int32.MaxValue) break;
                        else { Targets.Add(NextTarget); LightJumpHexes.Add(NextTarget.Hex); LastPoint = NextTarget.Hex; PassiveTargets.Remove(NextTarget); }
                    }
                    result.AddRange(RecieveDamage(art.Param1, WeaponGroup.Energy, LightJumpHexes.ToArray(), Targets));
                    break;
                case ArtefactAbility.BPhysicDamage:
                    Hex1 = CurBattle.Field.Hexes[hex1];
                    Targets = new List<ServerShipB>();
                    ...
                    result.AddRange(RecieveDamage(art.Param1, WeaponGroup.Physic, Hex1, Targets));
                    break;
                case ArtefactAbility.BIrregularDamage:
                    Targets = new List<ServerShipB>();
                    List<ServerShipB> restores = new List<ServerShipB>();
                    foreach (ServerShipB ship in CurBattle.BattleField.Values)
                    {
                        if (ship.States.Side == SSide) restores.Add(ship);
                        else if (ship.States.Indestructible == false) Targets.Add(ship); 
                    }
                    result.AddRange(RecieveDamage(art.Param1, WeaponGroup.Irregular,CurBattle.Field.Hexes[(byte)(CurBattle.Field.MaxHex / 2)], Targets));
                    foreach (ServerShipB ship in restores)
                        ship.Params.Health.AddParam(art.Param1);
                    break;
                case ArtefactAbility.BCyberDamage:
                   Hex1 = CurBattle.Field.Hexes[hex1];
                    ...
                    hexes.AddRange(CurBattle.Field.IntersectHexes[hex1, hex2, 1]);
                    foreach (byte b in hexes)
                    {
                        if (CurBattle.BattleField.ContainsKey(b) == false) continue;
                        if (CurBattle.BattleField[b].States.Indestructible == true) continue;
                        Targets.Add(CurBattle.BattleField[b]);
                    }
                    result.AddRange(RecieveDamage(art.Param1, WeaponGroup.Cyber, Hex1, Targets));
                    break;
            }
        ...
        public GameMove[] RecieveDamage(int ArtDamage, WeaponGroup group, byte[] points, List<ServerShipB> targets)
        {
            List<GameMove> result = new List<GameMove>();
            List<ServerShipB> DestroyedShips = new List<ServerShipB>();
            for (int i=0;i<targets.Count; i++)
            {
                int angle = CurBattle.Field.DistAngleRot[points[i], targets[i].Hex, 1, 1];
                bool destroyresult = targets[i].RecieveDamage(ArtDamage, group, angle);
                if (destroyresult == true) DestroyedShips.Add(targets[i]);
            }
            foreach (ServerShipB ship in DestroyedShips)
            {
        ...
        public GameMove[] RecieveDamage(int ArtDamage, WeaponGroup group, Hex hex1, List<ServerShipB> targets)
        {
            ...
            foreach (ServerShipB ship in targets)
            {
                ...
                if (destroyresult== true) DestroyedShips.Add(ship);
            }
            foreach (ServerShipB ship in DestroyedShips)
            {
                result.Add(new GameMove(14, ship.Hex, 255, 255));
        ...
        public void StartBattle()
        {
            foreach (ServerShipB ship in Ships.Values)
            {
                ...
                else if (ship.Hex < CurBattle.Field.MaxHex)
                {
                    OnField.Add(ship.BattleID);
                    CurBattle.BattleField.Add(ship.Hex, ship);
                    ship.Angle = (SSide == ShipSide.Attack ? 150 : 300);
                }
        ...
        public void RoundStart()
        {
            if (IsResurrect) //воскрешение корабля
            {
                ...
                foreach (ServerShipB ship in Ships.Values)
                {
                    if (ship.States.IsPortal || ship.States.BigSize || ship.States.Controlled==false) continue;
        ...
        public static void MakeMovesAuto(ServerBattle battle, ServerSide side1, ServerSide side2)
        {
            ...
            for (int j=0;j<Ships.Length; j++)
            {
                ...
                for (int i=0;i<3;i++)
                {
                    ...
                    if (moves!=null) foreach (GameMove move in moves)
                        if (move.IsTrue)
